...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    var list = db.activeProducts.toList()
    ...
    if (_lowOnly) {
      list = list
          .where((p) => p.trackInventory && db.stockOf(p.id) <= p.minQty)
          .toList();
    }
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          Padding(
            ...
            child: Center(
              child: Tag(
                'قيمة المخزون: ${app.inventory.stockValue().format()}',
              ),
            ),
          ),
        ],
                          ...
                          child: ListTile(
                            ...
                            title: Row(
                              children: [
                                ...
                                else if (p.daysToExpiry != null &&
                                    p.daysToExpiry! <= 30)
                                  Tag(
                                    'ينتهي خلال ${p.daysToExpiry} يوم',
  ...
  Future<void> _actions(BuildContext context, Product p) async {
          ...
          children: [
            ...
            if (p.trackInventory) ...[
              ListTile(
                ...
                title: const Text('إدخال كمية (وارد)'),
              ...
              ListTile(
                ...
                title: const Text('إخراج / هالك'),
                onTap: () => Navigator.pop(context, 'loss'),
              ),
              ListTile(
                leading: const Icon(Icons.tune),
                title: const Text('جرد (تحديد الكمية الفعلية)'),
                onTap: () => Navigator.pop(context, 'adjust'),
              ),
            ],
            ListTile(
              leading: Icon(Icons.qr_code_2_rounded, color: context.c.info),
              title: const Text('ملصق باركود للطباعة'),
              onTap: () => Navigator.pop(context, 'label'),
            ),
            ListTile(
              leading: Icon(Icons.delete_outline, color: context.c.danger),
    ...
    if (v == 'edit') {
      ...
      if (await confirm(
        ...
        await guarded(
          context,
          () => app.inventory.deleteProduct(p.id),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              SheetTitle(
                products.length == 1
                    ? 'ملصق: ${products.first.name}'
                    : 'ملصقات ${products.length} صنف',
              ),
              Text(
                'يُطبع لكل صنف: اسم المحل، اسم الصنف، الباركود (EAN-13 / Code-128) والسعر. الأصناف بدون باركود تحصل على كود داخلي قابل للمسح.',
                style: TextStyle(fontSize: 12, color: ctx.c.textMuted),
              ),
              const SizedBox(height: 12),
              SegmentedButton<LabelSize>(
                segments: [
                  for (final l in LabelSize.values)
                    ButtonSegment(
                      value: l,
                      label: Text(
                        l.label,
              ...
              Row(
                children: [
                  ...
                  IconButton.filledTonal(
                    onPressed: copies < 99 ? () => setS(() => copies++) : null,
                    icon: const Icon(Icons.add),
                  ),
                ],
              ),
              ...
              FilledButton.icon(
                ...
                icon: const Icon(Icons.qr_code_2_rounded),
                label: const Text('إنشاء الملصقات'),
              ),
            ],
          ),
        ),
  ...
  await showExportSheet(
    ...
    options: [
      ExportOption(
        ...
        build: () =>
            app.pdf.barcodeLabels(products, copies: copies, size: size),
      ),
    ],
  );
}
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'المخزون الحالي: ${db.stockOf(widget.product.id).format()} ${widget.product.unit}',
            style: TextStyle(color: context.c.textMuted),
          ),
          const SizedBox(height: 12),
          QtyField(
            controller: _qty,
            label: adjust ? 'الكمية الفعلية بعد الجرد *' : 'الكمية *',
            allowZero: adjust,
          ),
          const SizedBox(height: 12),
          TextFormField(
          ...
          FilledButton(
            onPressed: () async {
              ...
              final ok = await guarded(
                context,
                () => context.read<AppServices>().inventory.manualMove(
                  widget.product.id,
                  widget.type,
...
class _ProductFormSheetState extends State<ProductFormSheet> {
  ...
  late final _min = TextEditingController(
    text: widget.existing?.minQty.format(),
  );
  ...
  @override
  void dispose() {
    for (final c in [_name, _barcode, _unit, _buy, _sell, _min, _opening]) {
      c.dispose();
    }
    super.dispose();
  }
        ...
        children: [
                        ...
                        : IconButton(
                            ...
                            onPressed: () async {
                              ...
                              if (code != null && mounted) {
                                setState(
                                  () => _barcode.text =
                                      LedgerDb.normalizeBarcode(code),
                                );
                              }
                            },
                          ),
          ...
          ListTile(
            ...
            subtitle: _expiry == null
                ? const Text(
                    'اتركه فارغًا للمنتجات بلا صلاحية',
              ...
              Card(
                ...
                child: ListTile(
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () => setState(
                      () => _packUnits = [..._packUnits]..removeAt(i),
                    ),
                  ),
                ),
              ),
          ...
          FilledButton(
            onPressed: () async {
              if (!(_form.currentState?.validate() ?? false)) return;
              final app = context.read<AppServices>();
              Money m(TextEditingController c) =>
                  Money.tryParse(c.text) ?? Money.zero;
              Qty qq(TextEditingController c) =>
                  Qty.tryParse(c.text) ?? Qty.zero;
              final barcode = LedgerDb.normalizeBarcode(_barcode.text);
              Product? result;
              final ok = await guarded(context, () async {
                if (isEdit) {
                  result = await app.inventory.updateProduct(
                    widget.existing!.id,
                    name: _name.text,
                    barcode: barcode,
                    unit: _unit.text,
                    purchasePrice: m(_buy),
                    salePrice: m(_sell),
                    minQty: qq(_min),
                    trackInventory: _track,
                    packUnits: _packUnits,
                    expiryDate: _expiry,
                    clearExpiry: _expiry == null,
                  );
                } else {
                  result = await app.inventory.createProduct(
                    name: _name.text,
                    barcode: barcode.isEmpty ? null : barcode,
                    unit: _unit.text.trim().isEmpty ? 'حبة' : _unit.text,
                    purchasePrice: m(_buy),
                    salePrice: m(_sell),
                    minQty: qq(_min),
                    trackInventory: _track,
                    openingQty: qq(_opening),
                    packUnits: _packUnits,
                    expiryDate: _expiry,
                  );
                }
            ...
            onPressed: () {
              ...
              if (!(factor > Qty.one)) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('عدد الوحدات بالعبوة يجب أن يكون أكبر من 1'),
                  ),
                );
                return;
              }
