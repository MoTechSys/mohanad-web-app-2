...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final accurate = sum.profit(
      ProfitMode.accurate,
      cashPurchaseAsCogs: s.cashPurchaseAsCogs,
    );
    final estimated = sum.profit(
    ...
    final maxDay = days.fold(
      0,
      (m, d) => d.value.minor > m ? d.value.minor : m,
    );
          ...
          ExportButton(
            ...
            options: [
              ExportOption(
                ...
                subtitle:
                    'ملخص مالي، تدفق نقدي، أصناف، مصروفات، ديون، سجلات — باسم محلك وشعارك',
              ...
              ExportOption(
                ...
                subtitle:
                    'إيرادات، تكلفة، مجمل وصافي الربح، أعلى الأصناف ربحاً، المصروفات',
                icon: Icons.trending_up_rounded,
                fileBase: 'أرباح-$_label',
                build: () => app.pdf.monthlyProfitReport(
                  _r,
                  title: 'تقرير أرباح $_label',
                ),
              ),
              ExportOption(
                title: 'ملف Excel كامل',
                subtitle:
                    'أوراق: ملخص، مبيعات، أصناف، مشتريات، مصروفات، دخل يومي، عملاء، موردون، مخزون',
        ...
        child: ListView(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text(
                '$_label: ${Fmt.date(_r.start)} → ${Fmt.date(_r.end)}',
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    if (s.cashPurchaseAsCogs)
                      _row('مشتريات نقدية (كتكلفة)', -sum.cashPurchases),
                    ...
                    _row(
                      ...
                      accurate,
                      bold: true,
                      color: accurate.isNegative
                          ? context.c.danger
                          : context.c.primaryDark,
                    ),
                    _row(
                      'صافي الربح التقديري (كل المشتريات)',
                      estimated,
                      color: estimated.isNegative
                          ? context.c.danger
                          : context.c.info,
                    ),
                    const SizedBox(height: 6),
                    Text(
                      'الدقيق = الإيرادات − تكلفة البضاعة المباعة − المصروفات. التقديري يخصم كل المشتريات (نقدي + آجل) كتكلفة.',
                      style: TextStyle(
                        fontSize: 11,
            ...
            Row(
              children: [
                Expanded(
                  child: StatCard(
                    title: 'ديون جديدة بالفترة',
                    value: sum.newDebts.format(),
                ...
                Expanded(
                  child: StatCard(
                    ...
                    value: rep.customersDebt().total.format(),
                    color: context.c.danger,
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: StatCard(
                    ...
                    value: rep.suppliersDebt().total.format(),
                  ...
                  children: [
                    for (final p in top)
                      ListTile(
                        ...
                        title: Text(p.name),
                        subtitle: Text(
                          'كمية ${p.qty.format()} • ربح ${p.profit.format()}',
                        ),
