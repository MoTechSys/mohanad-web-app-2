...
class PdfBrand {
                ...
                children: [
                  ...
                  if (subtitle != null)
                    pw.Padding(
                      ...
                      child: pw.Text(subtitle, style: small),
                    ),
                ],
              ),
          ],
  ...
  pw.Widget footer(pw.Context ctx) {
    ...
    return pw.Column(
      ...
      children: [
        ...
        if (f.isNotEmpty)
          pw.Center(
            child: pw.Text(f, style: const pw.TextStyle(fontSize: 9.5)),
          ),
        ...
        pw.Row(
          ...
          children: [
            ...
            pw.Text(
              'طُبع ${Fmt.dateTime(DateTime.now())} • دفتر البقالة',
              style: small,
            ),
          ],
        ),
  ...
  pw.MultiPage page({
    ...
    header: (_) => pw.Padding(
      ...
      child: header(title: title, subtitle: subtitle),
    ),
    footer: footer,
    build: build,
  );
  ...
  pw.Widget table({
    ...
    pw.Widget cell(
      ...
      child: pw.Text(
        ...
        style: (head || tot)
            ? bold.copyWith(
                fontSize: 9.5,
                color: head ? PdfColors.white : primary,
              )
    ...
    return pw.Table(
      ...
      border: pw.TableBorder(
        horizontalInside: const pw.BorderSide(color: line, width: 0.4),
        bottom: const pw.BorderSide(color: line, width: 0.6),
      ),
      children: [
        pw.TableRow(
          ...
          children: [
            for (var i = 0; i < headers.length; i++)
              cell(headers[i], head: true, a: aligns?[i]),
          ],
        ),
        for (var r = 0; r < rows.length; r++)
          pw.TableRow(
            decoration: pw.BoxDecoration(
              color: r.isOdd ? zebra : PdfColors.white,
            ),
            children: [
              for (var i = 0; i < rows[r].length; i++)
                cell(rows[r][i], a: aligns?[i]),
            ],
          ),
        if (totals != null)
          pw.TableRow(
            ...
            children: [
              for (var i = 0; i < totals.length; i++)
                cell(totals[i], tot: true, a: aligns?[i]),
            ],
          ),
      ],
    );
  ...
  pw.Widget kpis(List<(String, String)> items, {int perRow = 3}) => pw.Wrap(
    ...
    children: [
      for (final (k, v) in items)
        pw.Container(
          width:
              (PdfPageFormat.a4.availableWidth - 56 - 8 * (perRow - 1)) /
              perRow,
