...
class ExcelExporter {
  ...
  void _sheet(Excel x, String name, List<String> headers, List<List<CellValue>> rows, {List<CellValue>? totals}) {
    final s = x[name];
    s.isRTL = true;
    s.appendRow([TextCellValue(db.settings.storeName)]);
    s.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0)).cellStyle = _bold;
    s.appendRow([TextCellValue('تاريخ التصدير: ${Fmt.dateTime(DateTime.now())}')]);
    s.appendRow([]);
    s.appendRow([for (final h in headers) TextCellValue(h)]);
    final hr = s.maxRows - 1;
    for (var i = 0; i < headers.length; i++) {
      s.cell(CellIndex.indexByColumnRow(columnIndex: i, rowIndex: hr)).cellStyle = _head;
    ...
    if (totals != null) {
      ...
      for (var i = 0; i < totals.length; i++) {
        s.cell(CellIndex.indexByColumnRow(columnIndex: i, rowIndex: tr)).cellStyle = _bold;
      }
    }
  }
  ...
  Future<Uint8List> periodWorkbook(DateRange r) async {
    ...
    _sheet(x, 'الملخص', ['البند', 'القيمة'], [
      [TextCellValue('الفترة'), TextCellValue('${Fmt.date(r.start)} — ${Fmt.date(r.end)}')],
      ...
      [TextCellValue('مقبوضات العملاء'), DoubleCellValue(_n(s.customerPayments))],
      [TextCellValue('مصروفات تشغيلية'), DoubleCellValue(_n(s.operatingExpenses))],
      ...
      [TextCellValue('دفعات الموردين'), DoubleCellValue(_n(s.supplierPayments))],
      [TextCellValue('تكلفة البضاعة المباعة'), DoubleCellValue(_n(s.cogs + s.manualCogs))],
      [TextCellValue('صافي الربح'), DoubleCellValue(_n(s.profit(st.profitMode, cashPurchaseAsCogs: st.cashPurchaseAsCogs)))],
    ...
    final sales = reports.salesIn(r).toList()..sort((a, b) => a.saleDate.compareTo(b.saleDate));
    _sheet(x, 'المبيعات', ['التاريخ', 'العميل', 'رقم الفاتورة', 'الدفع', 'المجموع', 'الخصم', 'الصافي', 'التكلفة', 'الحالة', 'ملاحظات'], [
      for (final v in sales)
        [
          TextCellValue(Fmt.dateTime(v.saleDate)),
          TextCellValue(v.customerId == null ? 'نقدي' : (db.customers[v.customerId!]?.name ?? '—')),
    ...
    ], totals: [TextCellValue('الإجمالي (النشطة)'), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), DoubleCellValue(_n(s.totalSales))]);
    ...
    _sheet(x, 'المشتريات', ['التاريخ', 'المورد', 'رقم الفاتورة', 'الدفع', 'المبلغ', 'الحالة', 'ملاحظات'], [
      for (final v in purchases)
        [
          ...
          TextCellValue(v.supplierId == null ? (v.supplierNameManual ?? '') : (db.suppliers[v.supplierId!]?.name ?? '—')),
          TextCellValue(v.invoiceNo ?? ''),
          TextCellValue(v.paymentType.label),
    ...
    final expenses = reports.expensesIn(r).toList()..sort((a, b) => a.expenseDate.compareTo(b.expenseDate));
    _sheet(x, 'المصروفات', ['التاريخ', 'النوع', 'الفئة', 'المورد', 'المبلغ', 'الحالة', 'البيان'], [
      for (final v in expenses)
        [
          TextCellValue(Fmt.dateTime(v.expenseDate)),
          TextCellValue(v.type.label),
          TextCellValue(v.categoryId == null ? '' : (db.categories[v.categoryId!]?.name ?? '')),
          TextCellValue(v.supplierId == null ? '' : (db.suppliers[v.supplierId!]?.name ?? '')),
    ...
    final incomes = db.dailyIncomes.values.where((d) => r.contains(d.incomeDate)).toList()..sort((a, b) => a.incomeDate.compareTo(b.incomeDate));
    _sheet(x, 'الدخل اليومي', ['التاريخ', 'المبلغ', 'تكلفة يدوية', 'الحالة', 'ملاحظات'], [
      for (final v in incomes)
        [
          TextCellValue(Fmt.date(v.incomeDate)),
          DoubleCellValue(_n(v.amount)),
          v.manualCogs == null ? TextCellValue('') : DoubleCellValue(_n(v.manualCogs!)),
  ...
  Future<Uint8List> customersWorkbook() async {
    ...
    _customersSheet(x);
    x.delete('Sheet1');
    final tx = db.customerTx.values.toList()..sort((a, b) => a.txDate.compareTo(b.txDate));
    _sheet(x, 'حركات العملاء', ['التاريخ', 'العميل', 'النوع', 'المبلغ', 'الرصيد قبل', 'الرصيد بعد', 'الحالة', 'ملاحظات'], [
      for (final t in tx)
        [
  ...
  Future<Uint8List> inventoryWorkbook() async {
    ...
    _stockSheet(x);
    x.delete('Sheet1');
    final moves = db.stockMoves.values.toList()..sort((a, b) => a.createdAt.compareTo(b.createdAt));
    _sheet(x, 'حركات المخزون', ['التاريخ', 'الصنف', 'النوع', 'التغير', 'قبل', 'بعد', 'الحالة', 'ملاحظات'], [
  ...
  void _customersSheet(Excel x) {
    final cs = db.activeCustomers.toList()..sort((a, b) => db.customerBalance(b.id).minor.compareTo(db.customerBalance(a.id).minor));
    _sheet(x, 'العملاء', ['الاسم', 'الهاتف', 'العنوان', 'الرصيد (دين)', 'حد الائتمان', 'الحالة', 'ملاحظات'], [
      for (final c in cs)
        [
          ...
          c.creditLimit == null ? TextCellValue('') : DoubleCellValue(_n(c.creditLimit!)),
          TextCellValue(c.status.label),
          TextCellValue(c.notes ?? ''),
        ],
    ], totals: [TextCellValue('الإجمالي'), TextCellValue(''), TextCellValue(''), DoubleCellValue(_n(cs.fold(Money.zero, (p, c) => p + db.customerBalance(c.id))))]);
  }

  void _suppliersSheet(Excel x) {
    final ss = db.activeSuppliers.toList();
    _sheet(x, 'الموردون', ['الاسم', 'الهاتف', 'المستحق له'], [
      for (final s in ss) [TextCellValue(s.name), TextCellValue(s.phone ?? ''), DoubleCellValue(_n(db.supplierBalance(s.id)))],
    ], totals: [TextCellValue('الإجمالي'), TextCellValue(''), DoubleCellValue(_n(ss.fold(Money.zero, (p, s) => p + db.supplierBalance(s.id))))]);
  }

  void _stockSheet(Excel x) {
    final ps = db.activeProducts.toList()..sort((a, b) => a.name.compareTo(b.name));
    ...
    _sheet(x, 'المخزون', ['الصنف', 'الباركود', 'الوحدة', 'الكمية', 'حد النقص', 'سعر الشراء', 'سعر البيع', 'القيمة', 'تنبيه'], rows,
        totals: [TextCellValue('الإجمالي'), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), DoubleCellValue(_n(value))]);
  }
}
