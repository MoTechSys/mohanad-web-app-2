  ...
  group('ورديات الصندوق (م3)', () {
    test('فتح وردية برقم متسلسل Z-0001 ثم منع وردية ثانية', () async {
      ...
      final s = await app.shifts.openShift(
          workerName: 'محمد', openingCash: m(5000));
    ...
    test('اسم العامل مطلوب والافتتاحي لا يكون سالبًا', () async {
      final app = await boot();
      expect(() => app.shifts.openShift(workerName: '  ', openingCash: m(1)),
          throwsCode(ErrorCodes.invalidAmount));
      expect(
          () => app.shifts
              .openShift(workerName: 'س', openingCash: Money(-100)),
          throwsCode(ErrorCodes.invalidAmount));
    });
    ...
    test('تقرير Z يجمع النقد الداخل والخارج ويحسب المتوقع', () async {
      final app = await boot();
      final s = await app.shifts.openShift(
          workerName: 'أحمد', openingCash: m(10000));
      ...
      // سند قبض خارجي 200
      await app.vouchers
          .createReceipt(partyNameManual: 'جهة', amount: m(200));
      // مصروف 800
      await app.documents.createExpense(
          type: ExpenseType.other, amount: m(800), details: 'كهرباء');
    ...
    test('الإغلاق يجمّد المتوقع ويحسب الفرق ويمنع إغلاقًا مزدوجًا', () async {
      final app = await boot();
      final s = await app.shifts.openShift(
          workerName: 'سالم', openingCash: m(1000));
      await app.documents.createSale(
          paymentType: PaymentType.cash,
          mode: DocMode.totalOnly,
          totalAmount: m(500));
    ...
    test('الحركات خارج نافذة الوردية لا تدخل التقرير', () async {
      ...
      await app.documents.createExpense(
          type: ExpenseType.other, amount: m(999), details: 'قديم');
      final s = await app.shifts
          .openShift(workerName: 'ع', openingCash: m(0));
      final r = app.shifts.zReport(s);
      expect(r.expenses, Money.zero);
      expect(r.expectedCash, Money.zero);
    });

    test('الاستمرارية: الورديات تبقى بعد إعادة التحميل وضمن النسخة الاحتياطية',
        () async {
      final backend = MemoryBackend();
      var app = AppServices.withBackend(backend);
      await app.init();
      final s = await app.shifts.openShift(
          workerName: 'دائم', openingCash: m(700));
