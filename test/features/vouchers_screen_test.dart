...
void main() {
  testWidgets('vouchers screen shows empty state then lists a voucher',
    ...
    await app.vouchers.createReceipt(
        partyNameManual: 'أحمد', amount: Money.units(500));
    await tester.pumpAndSettle();
  ...
  testWidgets('create a receipt via the form for a manual party',
    ...
    await tester.enterText(
        find.widgetWithText(TextFormField, 'استلمنا من (الاسم)'), 'صالح');
    await tester.enterText(
        find.widgetWithText(TextFormField, 'المبلغ'), '750');
  ...
  testWidgets('filter separates receipts from payments', (tester) async {
    final app = await boot();
    await app.vouchers
        .createReceipt(partyNameManual: 'قبض١', amount: Money.units(100));
    await app.vouchers
        .createPayment(partyNameManual: 'صرف١', amount: Money.units(200));
  ...
  group('SMS templates', () {
    test('credit sale message includes amount and total debt', () async {
      ...
      final msg = app.sms.creditSaleMessage(
          saleAmount: Money.units(500), totalDebt: Money.units(1500));
    ...
    test('receipt message shows remaining or full settlement', () async {
      final app = await boot();
      final partial = app.sms
          .receiptMessage(paid: Money.units(300), remaining: Money.units(200));
      expect(partial, contains('المتبقي عليكم'));
      final full = app.sms
          .receiptMessage(paid: Money.units(300), remaining: Money.zero);
    ...
    test('notifyVoucher sends the customer paid + remaining', () async {
      ...
      final c = await app.parties.createCustomer(
          name: 'سمير', phone: '777123456', openingBalance: Money.units(1000));
      final v = await app.vouchers
          .createReceipt(customerId: c.id, amount: Money.units(400));
    ...
    test('reminder is skipped when there is no debt or phone', () async {
      final app = await boot();
      final noPhone =
          await app.parties.createCustomer(name: 'بدون رقم', openingBalance: Money.units(100));
      app.sms.sender = (_, _) async => true;
      expect(await app.sms.sendReminder(noPhone.id), isFalse);

      final noDebt =
          await app.parties.createCustomer(name: 'مسدد', phone: '777000000');
