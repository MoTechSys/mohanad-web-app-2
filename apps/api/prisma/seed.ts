...
async function main(): Promise<void> {
  ...
  // ─── 2. Permissions ──────────────────────────────
  console.log(`📋 Seeding ${ALL_PERMISSION_CODES.length} permissions...`);
  for (const code of ALL_PERMISSION_CODES) {
    const meta = describePermission(code);
    const groupAr = PERMISSION_GROUPS_AR[meta.module] ?? meta.module;
  ...
  console.log(`✓ Permissions ready: ${ALL_PERMISSION_CODES.length}`);

  // ─── 3. Roles + RolePermissions ─────────────────
  console.log('👤 Seeding system roles...');
  const ownerName: string = SYSTEM_ROLE_NAMES.OWNER;
  ...
  if (!ownerRole) throw new Error('Owner role missing — seed corrupted');

  const passwordHash = await bcrypt.hash(env.ownerPassword, env.bcryptRounds);
